...
"#,
    ...
    let memory = MemoryRecord {
        ...
        body: "Agents should issue a focused explicit recall query when prior context is needed."
            .to_string(),
    ...
    assert!(stdout.contains("focused explicit recall query"));
    server.join();
    ...
    assert!(markdown.contains("focused explicit recall query"));
...
#[test]
fn bare_recall_requires_an_explicit_query() {
    let tmp = TempDir::new().unwrap();
    let home = tmp.path().join("home");
    let project = tmp.path().join("project");
    fs::create_dir_all(home.join(".yaaml")).unwrap();
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("recall requires --query"));
}
