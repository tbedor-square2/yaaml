...
#[test]
fn recall_query_diversifies_near_duplicate_memories() {
    let tmp = TempDir::new().unwrap();
    let home = tmp.path().join("home");
    let project = tmp.path().join("project");
    fs::create_dir_all(home.join(".yaaml")).unwrap();
...
"#,
    ...
    let mut db = Database::open(&db_path).unwrap();
    db.migrate().unwrap();
    let insert = |db: &Database, title: &str, body: &str, vector: &[f32]| {
        let memory_id = db
            .insert_memory(&MemoryRecord {
                id: None,
    ...
    insert(
        &db,
        "Functional style preference in Java",
        "When editing Java transformations, prefer stream-style functional code over imperative loops.",
        &[0.8, 0.6],
    );
    ...
    let output = Command::new(binary)
        .arg("recall")
        .arg("--query")
        .arg("turn context recall ranking")
        .current_dir(&project)
        .env("HOME", &home)
        .env("OPENAI_API_KEY", "test-key")
        .env_remove("CODEX_THREAD_ID")
    ...
    server.join();
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert_eq!(stdout.matches("## Backtest").count(), 1);
    assert!(stdout.contains("## Functional style preference in Java"));
}
