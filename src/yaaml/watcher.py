...
CLAUDE_WATCH_PATH = Path("~/.claude/projects/").expanduser()
CODEX_WATCH_PATH = Path("~/.codex/sessions/").expanduser()
...
def _persist_session(db: sqlite3.Connection, meta: SessionMeta) -> None:
    ...
    existing = db.execute(
        "SELECT id FROM sessions WHERE id = ?", (meta.session_id,)
    ).fetchone()
    if existing:
        db.execute(
...
def _persist_turn(db: sqlite3.Connection, turn: ParsedTurn) -> None:
    ...
    def _ins(role: str, content_dict: dict) -> None:
        db.execute(
...
class FileWatcher:
    ...
    async def process_file(self, file_path: Path) -> None:
        ...
        is_codex = CODEX_WATCH_PATH.resolve() in [
            p for p in file_path.resolve().parents
